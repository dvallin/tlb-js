...
export function attackTarget(world: TlbWorld, random: Random, entity: Entity, target: Entity, attack: Attack): void {
  const chance = calculateHitChance(world, entity, target, attack)

  const log: Log = world.getResource<LogResource>('log')
  log.attack(world, entity, target, attack)
  if (random.decision(chance)) {
